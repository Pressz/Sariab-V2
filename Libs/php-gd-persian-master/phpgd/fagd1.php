...
class FAGD
{
    ...
    function GetNastaliq()
    {
        return $nastaligh = array(
            'ه' => array ('ﮫ', 'ﮭ', 'ﮬ')
        );
    }
    function GetNormal()
    ...
    function GetMPChars()
    {
        return $mp_chars = array ('آ', 'ا', 'د', 'ذ', 'ر', 'ز', 'ژ', 'و', 'أ', 'إ', 'ؤ');
    }
    function GetIgnoreList()
    {
        return $ignorelist = array('','ٌ','ٍ','ً','ُ','ِ','َ','ّ','ٓ','ٰ','ٔ','ﹶ','ﹺ','ﹸ','ﹼ','ﹾ','ﹴ','ﹰ','ﱞ','ﱟ','ﱠ','ﱡ','ﱢ','ﱣ',);
    }
    

    function Do ($str,$z="",$method='normal'){
        $p_chars = $this->GetPChars();
        $mp_chars = $this->GetMPChars();
        $ignorelist = $this->GetIgnoreList();
        $nastaligh = $this->GetNastaliq();
        ...
        for ($i=0; $i<$str_len; $i++){
            $str1=$ar[0][$i];
            if(    isset($ar[0][$i+1])and    in_array($ar[0][$i+1],$ignorelist)){
                $str_next=$ar[0][$i+2];
                if ($i == 2) $str_back=$ar[0][$i-2];
                if ($i != 2) $str_back=$ar[0][$i-1];
            }elseif(    isset($ar[0][$i-1])and    !in_array($ar[0][$i-1],$ignorelist)){
                if (isset($ar[0][$i+1]))
                    $str_next=$ar[0][$i+1];
